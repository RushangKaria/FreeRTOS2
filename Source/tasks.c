...
typedef struct tskTaskControlBlock
{
	...
	#if (configREAL_TIME_TASK == 1 )
	xTimerHandle handle;	//for real time clocks
	#endif
 ...
 */
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed char * const pcName, unsigned portBASE_TYPE uxPriority,unsigned int deadline, const xMemoryRegion * const xRegions, unsigned short usStackDepth ) PRIVILEGED_FUNCTION;
...
signed portBASE_TYPE xTaskGenericCreate(pdTASK_CODE pxTaskCode,
	...
	void *pvParameters, unsigned portBASE_TYPE uxPriority, unsigned int deadline,
	xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer,
	const xMemoryRegion * const xRegions)//added deadline parameter
 {
...
tskTCB * pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		prvInitialiseTCBVariables( pxNewTCB, pcName, uxPriority,deadline, xRegions, usStackDepth );
...
void vTimerReset(xTaskHandle pxTask)
{
tskTCB *pxTCB;

#if (configREAL_TIME_TASK == 1)
	pxTCB=prvGetTCBFromHandle(pxTask);
	xTimerReset(pxTCB->handle,0);	
	xTimerStart(pxTCB->handle,0);
#endif

return;
}

void vTimerStop(xTaskHandle pxTask)
{
tskTCB *pxTCB;

#if (configREAL_TIME_TASK == 1)
	pxTCB=prvGetTCBFromHandle(pxTask);	//stop the timer
	xTimerStop(pxTCB->handle,0);
#endif
...
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed char * const pcName, unsigned portBASE_TYPE uxPriority,unsigned int deadline, const xMemoryRegion * const xRegions, unsigned short usStackDepth )
{
...
*/
	...
	if(deadline>0)
	{
	pxTCB->handle=xTimerCreate((signed char *)pcName,deadline,pdFALSE,x,dummy);
	xTimerStart(pxTCB->handle,0);
	}
#endif
